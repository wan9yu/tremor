...
| entsog_gas_flow (energy) | **R32: BUILT then REJECTED on the IC gate** — not on source (ENTSOG operational-data is keyless/daily/~1d-lag, guard/cadence/reachability all pass), on BASE RATE. The worst-of / own-baseline-fraction construct transplanted from net_bgp_withdrawal is DEGENERATE for gas: physical flow at a single cross-border/LNG entry point is inherently LUMPY (LNG arrives in discrete cargoes → flow → ~0 between them; pipes swing on commercial nominations and seasonal routing), so a deep single-point fractional drop is common and benign. Measured on a full 2021→2026 seed (252 points cached): single-day worst-of fires **97.5%** of days (median worst-fraction 0.00); the zombie-proposed sustained 3/5/7-day fix fires 94.6/89.8/86.3%; even a strict steady-corridor list (24 pts, share≥0.99, reldisp≤0.4) at the deepest cutoff (frac≤0.05, a 95% cut) still fires **16.8%**, and there its LARGEST episodes are 2021 PRE-WAR seasonal LNG lumpiness — the real cuts (Yamal, NS1, Ukraine transit halt) are present but smaller than and indistinguishable from the benign noise. The R20/R32 "reachability" was spurious: the events produce frac=0, but so does nearly every day (reachability without a base-rate check ≠ IC). No cutoff, steadiness filter, or sustain window rescues it — a min-over-N-lumpy-series crosses any fixed bar near-daily (net_bgp survives this ONLY because route-counts are rock-steady). The energy axis stays un-spanned; per **Coverage is not a goal**, that is preferable to registering a factor that fires on everything. A DIFFERENT construct — worst-of across SOURCE-COUNTRY aggregates (Russia-total, Norway-total, LNG-total…), where within-source aggregation kills the lumpiness while worst-of across sources avoids dilution — is the only path worth a future build; it needs a live adjacency fetch + full re-aggregation + a fresh base-rate/reach test (a different line). Seed cache + measurements banked in session scratch so the negative is not re-run. |
...
3. **Decay / neutralization.** A factor must carry its FRESH signal — fire on the ONSET of disorder,
   ...
   BETA; an event-FREQUENCY concentration is structural and cannot be residual-cleaned (R32 probe A,
   net_bgp_withdrawal).
...
rejection. The construct fits steady-baseline entities, not lumpy ones.

**No doom score.** One factor firing is usually local; several independent factors firing in their own
alarm direction at once — resonance — is what "more disordered" looks like. The panel is never
...
- **Round 32** — 2026-09-10 · the factor-model round: two probes run, both FAIL, and the DOCTRINE is
  ...
  → CLOSED-AS-STRUCTURAL:** SY/IQ/SD are an event-frequency concentration (17/28 onsets, 60.7%), not a
  subtractable level beta — no residual cleans it; comms slot stays open. **Probe B (build entsog_gas_flow,
  the energy axis) FAILS on the IC gate → REJECTED:** the worst-of/own-baseline construct is degenerate
  for lumpy gas flow (single-day fires 97.5%, median worst-frac 0.00; strict steady list at a 95%-cut
  cutoff still 16.8%, its largest episodes benign 2021 seasonal lumpiness). Probe B's earlier
  "reachability" was spurious — reach without a base-rate check ≠ IC. The build was fully reverted (no
  registration); energy stays un-spanned per **Coverage is not a goal**. The doctrine's sharpening —
  coverage is not a goal, an un-spanned axis beats a manufactured factor, heavy scaffolding to clear a
  gate is the gate failing quietly — is R32's durable output. Docs-only; no line moved, no scoring
  changed (replay --check 0-divergence, no STABLE_SINCE bump). Process note: an implementer subagent
  again backgrounded a long fetch + sleep-poll shells and abandoned mid-task (the R28 pattern); the
  controller took ownership, ran the fetch cleanly, and adjudicated the probe. radar-metrics regenerated.
