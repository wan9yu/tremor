...
| net_bgp_withdrawal | communications (global) | 2 | 3 | 3 | **BUILT R30** into the empty communications slot as the route-WITHDRAWAL candidate the R29 probe validated (evidence data/archive/bgp_probe_2026-09-08.csv). Passive-BGP worst-of route visibility: a size-floored 153-country watch-list (median daily-median visible-/24 ≥ 512), each country's daily-min ÷ its 28-day rolling-median baseline, worst-of across the list, scored DOWN in anchored scale-mode (ANCHOR=1.0, MATERIALITY=0.20 → alarm at fraction 0.40, a ≥60% withdrawal, well below the ~0.75 structural benign floor). Fires strongly on the cited events — Syria 2022-05-30 frac 0.031 → z−4.84, Sudan 2023-04-24 frac 0.314 → z−3.43 — and is structurally immune to the active-probing common-mode that demoted net_outages (BGP is a passive read of route announcements, no probe vantage to lose). Orthogonal (probe |max Pearson| 0.167 vs flights). Enters at tier-2 and earns tier-1 over the ≥60-scored funnel, NOT by declaration. **CALIBRATION (corrected R31):** the R30 build set MATERIALITY=0.10 (alarm frac<0.70) to match the R29 probe's rolling-Qn fired/benign separation (~5% on the recent calm window), but on the anchored full seed that bar fired ~21% (352/1673 days) — it flagged CHRONIC route-withdrawal states each day (Sudan 61 / Iraq 58 / Syria 51 ≈ 48% of runs, Cameroon 40) rather than the withdrawal events the line exists to catch, so the 0.10 never achieved its intended separation. R31 tightened MATERIALITY to 0.20 (alarm frac<0.40, a ≥60% withdrawal) and re-scored the committed seed — raw_value, the measurement, unchanged; only z_score/trembling recompute: fire 21% → 8.7% (145/1673 down-trembles), BOTH cited events preserved (0.20 is the CHOSEN value, not a hard tightest — any materiality ≤ ~0.229 still fires Sudan at frac 0.314: 0.22 → z −3.12, 0.23 or tighter drops it). This resolved the BASE-RATE over-fire ONLY: chronic states SY/IQ/SD still drive ~72% of the 145 fires (49/39/16, UP from ~48% at 0.10), so chronic-state handling was carried as the pre-promotion item. **R32 probe A — chronic contamination CLOSED (corrected by fable R32):** SY/IQ/SD dominate the fires because those ARE the world's most chronically-disordered route tables — that concentration is the withdrawal factor's DISTRIBUTION, not a beta to subtract. A per-country level residual does not clean it without cost: a short-window residual (7–14d) cuts chronic share from 71.7% to ~50–53% but at 7d it LOSES the Sudan reference (z −2.86, sub-alarm); a longer window keeps Sudan but leaves lag-1 autocorr at 0.49–0.52 — never down into the counted tier-1 lines' fresh band (flights 0.31 / cnh_cny 0.27 / fed_srf 0.41; cf. Known limit 3's 0.07–0.33), chronic share ~68%. So the honest statement is not 'un-neutralizable' in the abstract but 'not level-cleanable without losing the event or the freshness'. A breadth or per-episode CAP is UNLIKELY to help either — the Sudan 2023-04-24 reference is a single-country event at frac 0.314, so any ≥2-country breadth bar would kill it, and a per-country cap makes the operator path-dependent (a STABLE_SINCE-bumping DIFFERENT line, not a tuning of this one). net_bgp stays tier-2 at MAT=0.20 as-is; the communications slot stays OPEN and disclosed. **Promotion status (fable R32):** the line is past the ≥60-scored funnel (1673 scored, 8.7% fire) but is NOT promotion-ready — its fires are chronic-state-dominated, a disclosure the counted headline should not carry; it is held at tier-2 as a communications WATCHLIST candidate, and promotion would need the fresh-onset construct above, not this line as built. A pre-promotion calibration of the tier-2 candidate, never counted — forward-only (replay --check 0-divergence, no STABLE_SINCE bump, no summary change; pre-correction 0.10 scoring preserved at commit 7f0eb08); see the date-sorted 2026-09-09 annotation. **DISCLOSURE (route-WITHDRAWAL only, the gnss "effective reach 1" precedent):** BGP measures route ANNOUNCEMENT, not reachability — it catches shutdowns/cable-cuts/transit failures that WITHDRAW prefixes but MISSES access-layer blackouts where routes stay announced. Gaza 2023-10 total blackout read only ~−4% (PS frac ~0.96, z≈−0.4, never near the alarm) — the ASes kept announcing into the dark. It does not close the slot's full "is the country reachable" question, only the withdrawal subclass |
...
| entsog_gas_flow (energy) | **R32: BUILT then REJECTED on the IC gate** — not on source (ENTSOG operational-data is keyless/daily/~1d-lag, guard/cadence/reachability all pass), on BASE RATE. The worst-of / own-baseline-fraction construct transplanted from net_bgp_withdrawal is DEGENERATE for gas: physical flow at a single cross-border/LNG entry point is inherently LUMPY (LNG arrives in discrete cargoes → flow → ~0 between them; pipes swing on commercial nominations and seasonal routing), so a deep single-point fractional drop is common and benign. Measured on a full 2021→2026 seed (252 points cached): single-day worst-of fires **97.5%** of days (median worst-fraction 0.00); the zombie-proposed sustained 3/5/7-day fix fires 94.6/89.8/86.3%; even a strict steady-corridor list (24 pts, share≥0.99, reldisp≤0.4) at the deepest cutoff (frac≤0.05, a 95% cut) still fires **16.8%**, its episodes benign (the largest a 2023 Norwegian maintenance season, then 2021 seasonal LNG lumpiness — all in the calm direction). **Correction (fable R32):** the salvage did NOT show "the real cuts are present but indistinguishable" — it showed the OPPOSITE and worse: the watch-lists were floored on a RECENT window (last-120d as-built, last-365d steady), which SURVIVORSHIP-excludes every corridor that has since died (Mallnow/Yamal, both Greifswald/NS1 points, Velké Kapušany all read 0 today), so the cited cuts were never in the measured set at all — the steady-24 list is entirely LNG terminals + intra-EU VIPs and fires ZERO in the Ukraine-halt window. The salvage therefore tested whether LNG/VIP points are lumpy (yes), not whether a real cut is distinguishable; and the ex-post derivation is itself the lookahead the Point-in-time rule forbids. The reject still holds on the decisive full-list 97.5%. No cutoff, steadiness filter, or sustain window rescues the per-point worst-of — a min-over-N-lumpy-series crosses any fixed bar near-daily (net_bgp survives this ONLY because route-counts are rock-steady). Per **Coverage is not a goal**, the energy axis is left un-spanned rather than registering a factor that fires on everything — but it is UNTESTED, not proven un-spannable: the cheapest untried construct (recorded below, NOT run this round per the step-back) is a SINGLE summed series — EU total pipeline entry ex-LNG, or a Russia-corridor total (the five cited points are known by key, so NO adjacency fetch is needed) — whose dilution was asserted from the docstring, never measured. The energy axis now holds TWO dead candidates on different gates: this (IC) and eu_gas_storage (parked R24 on reachability). Seed cache banked in session scratch (the measurement scripts import the reverted fetcher, so they are not re-runnable as-is; the cache is the durable artifact). |
...
| airspace human-hand axis (per-region flights / conflict-corridor) | **R34: REJECTED on COVERAGE — un-spannable on the TESTED conflict zones with keyless community ADS-B.** The goal was to reconceive `flights` from a weather-suppressing worldwide sum into a probe for the invisible HUMAN hand (military / geopolitical airspace closures). Two constructs, both rejected by measurement. (1) **Per-region worst-of** (un-sum the 4 regions, score each on its own z): REJECTED — the surfaced "signal" (a 3-week E-Asia drop read as Typhoon Dolphin) was a SAMPLE-HOUR PHASE SEAM (a ~30-min GH-Actions cron drift on Japan's ~100-ac/hr morning ramp = the whole −50); precision 0/15 on the 64-day window, and the "sustained" discriminator is inverted (artifacts are sustained, real weather single-day). (2) **Conflict-corridor closure** (worst-of of a corridor box vs a neighbor control): REJECTED — keyless community ADS-B coverage is driven by volunteer ground receivers, which cluster in stable regions and are STRUCTURALLY ABSENT over conflict zones. Verified on keyless historical snapshots (adsbexchange readsb-hist, monthly-1st): central Ukraine (Kyiv) read ~1 airborne aircraft at noon PRE-war (2022-01/02) vs Poland 22 / Germany 69 / France 74 same instant — a COVERAGE FLOOR, not traffic — and ~1 during the war too, so a closure there is indistinguishable from the floor; Israel single-digit to low-teens historically — too thin to score. The live "Ukraine=1" first read as an incontrovertible closure was that same coverage floor. Where coverage IS dense (W Europe / US / hubs) closures essentially never happen, and a loose bbox dilutes with border traffic (the Ukraine war-closure moved a loose bbox only ~2×, below any alarm). SCOPE of the negative: only Ukraine + Israel were BACKTESTED historically; the live-admitted Gulf (86) and Korea (79) boxes are coverage-adequate but were NOT backtested, and the monthly-1st-noon snapshot method structurally cannot see transient hour-to-day closures (the 2024–25 Gulf/Levant closures are that class) — so those boxes are un-BUILT-here, not proven un-spannable, though the phase-trap objection still applies to any fixed-hour construct over them. So `flights` STAYS the single summed weather-suppressing aggregate (its high bar rightly ignores thunderstorms — the guard it exists for). BANKED so the search is not repeated: (a) keyless historical ADS-B via adsbexchange readsb-hist monthly-1st snapshots is a real backtest tool for STANDING closures / coverage floors (NOT transient ones) in future probes; (b) the phase-seam and coverage-floor confounds are the two traps any future ADS-B geometry must clear first. Design + backtest detail in internal/2026-09-11 + 2026-09-13. |
...
## tremor is a factor library for world disorder

Each line is an OPERATOR over a raw world source → a normalized disorder reading. The registry is
curated like a quant factor model: a spanning set of low-correlation factors, each with a structural
rationale, validated empirically, read as a PANEL — never summed into one doom score. The two tiers,
the three gates, and the metrics are all instruments of this one discipline. A factor lives or dies
by the PROBE, never by taxonomy or gut (data-backed, never 拍脑袋). Four tests decide it — every
candidate, and periodically every serving line, runs them:

1. **Rationale — the guard gate.** A defended equilibrium with a hand that can leak. No guard → a
   SYMPTOM (an exogenous event: a volcano, a cyclone), context/contrast only, never counted.
2. **IC — the coincidence probe, standing.** Of the days a factor fires in its alarm direction, what
   share are adjudicated REAL disorder vs artifact (precision), and does it catch its domain's real
   events at all (recall)? Reachability without a base-rate check is NOT IC: a reading that fires on
   the event AND on nearly every calm day is a decoration (net_outages demoted here; the R32 entsog
   per-point construct rejected here — each "reached" the event but fired near-daily).
3. **Decay / neutralization.** A factor must carry its FRESH signal — fire on the ONSET of disorder,
   not on a chronic state. But a chronic component is removable only when it is a subtractable level
   BETA; where a factor's fires CONCENTRATE in the few entities that are chronically most disordered,
   that concentration is the factor's DISTRIBUTION, not contamination to subtract — a level residual
   reduces it only by losing the reference event or the freshness (R32 probe A, net_bgp_withdrawal).
4. **Orthogonality + spanning.** Low pairwise |correlation| — a factor near-spanned by another
   (|r|~0.7+) is cut (em_corp_oas / euro_hy from the credit slot). A new factor is admitted only for
   MARGINAL ORTHOGONAL IC.

**Coverage is not a goal** (sharpened R32). The library's health is orthogonal IC PER FACTOR, not
dimensional completeness. An un-spanned axis is honest information — some disorder dimensions have no
daily-cadence, guard-gated, keyless probe right now — and beats a manufactured weak factor filling
the hole. If a factor needs heavy scaffolding (stacked filters, sustain windows, re-aggregation) to
barely clear a gate, that is the gate failing quietly, not a signal — reject it. Discipline is
curatorial and SUBTRACTIVE; the factor-zoo / coverage impulse is the trap it exists to refuse.
Spanning (test 4) is a DIRECTION, not a quota: marginal orthogonal IC is the gate, and an un-spanned
axis is a fact to DISCLOSE, not a slot to fill. The tier-1 target of 4 is a DISPLAY budget, not a
factor-count principle — a disclosed open slot beats a fourth line added for the count (R30 filled the
slot with a fourth financial line, so the counted set is now finance-weighted: an honest cost, not a
win). And an axis is called un-spanned only AFTER the cheapest single-series construct has a MEASURED
base rate — asserting degeneracy or dilution from reasoning alone is the tropical_cyclone error; short
of that test the axis is UNTESTED, not un-spannable.

**Standard aggregation, and its limit.** When a domain is a basket of entities, the non-diluting form
is a worst-of / own-baseline-fraction / anchored-scale construct — per-entity day-value ÷ its own
rolling-median baseline, worst-of across a size-floored watch-list, anchored scale-mode
(net_bgp_withdrawal is the surviving instance). It is NOT universal: worst-of over inherently LUMPY
series (gas physical flow per point) is a min-over-noise that fires near-daily — the R32 entsog
rejection. The construct fits steady-baseline entities, not lumpy ones. Two cautions it hides.
(a) BASELINE MEMORY is a design choice, not a free virtue: a per-entity baseline that decays to
drop-out (an entity dead past the window returns None) self-heals against chronic states but then
FORGETS a permanent cut after one window; a frozen baseline keeps chronic states forever. Neither is
a default — the line must state which it assumes. (b) The WATCH-LIST derivation is itself
point-in-time: a list floored on a RECENT window silently drops entities that have since died, so a
back-test over it is survivorship (the R32 entsog salvage floored on the last 120 days and thereby
excluded the very corridors whose cut it meant to test — the lookahead the Point-in-time rule forbids).

**No doom score.** One factor firing is usually local; several independent factors firing in their own
alarm direction at once — resonance — is what "more disordered" looks like. The panel is never summed
into a weighted score; the COUNT of independent fires is the only aggregate.

**Point-in-time.** STABLE_SINCE + replay are the no-lookahead rule: verdicts re-derive from the raw
source under today's rules from STABLE_SINCE forward; a scoring change bumps it, a bug is rescored, a
measurement is never rewritten (forward-only).
...
- **flights sample-hour fix — SHIPPED R25.1** (opened R25, which CLOSED the R11 pre-committed
  ...
  baseline is continuous (no reseed) — TRUE FOR THE TOTAL, but NOT for a per-region SHARE: R34 found
  Japan's share moved with a ~30-min cron drift across ~08-08 (a diurnal-ramp phase seam), so any
  future per-region reformulation cannot treat this baseline as continuous. A backstop guard (`collect.py apply_sample_guard`, declared
...
- **level-layer → flights** (opened R23.1) — decide ~Nov 2026 (once flights per-region components
  ...
  rows_since(flights)≥60, now ~10/60). **R34 KILLS the planned per-region fix.** The Nov plan was a
  per-region **worst-of-region z** (its cited −7.84z / −7.33z were PHASE-SEAM artifacts — a ~30-min
  cron drift on a diurnal ramp, not signal), and R34 rejected the whole per-region reconception:
  un-summing exposes sample-hour phase seams (precision 0/15 on the 64-day window), and its successor,
  a conflict-corridor closure line, was rejected too — keyless community ADS-B coverage is
  structurally absent over conflict zones (central Ukraine reads ~1 aircraft pre-war AND during-war =
  a coverage floor, not a closure; Israel ~0), so a closure is indistinguishable from the floor (see
  Rejected + Round 34). The airspace human-hand axis is un-spannable on the tested conflict zones via keyless ADS-B; flights stays
  the single summed weather-suppressing aggregate (its high bar rightly ignores thunderstorms). The
  per-region reformulation and the second-sample-hour are both KILLED R34. This tripwire now covers
  ONLY the remaining question — whether to extend the level layer to flights (still motivated by the
  stale, panel-churning PortWatch second headline), which is a display-layer choice, NOT a
  flights-scoring change. [opened R23.1 · owner R26 · fires: date >= 2026-11-01]
...
- **Round 31** — 2026-09-09 · net_bgp_withdrawal calibration correction of the R30 build: anchored
  MATERIALITY 0.10 → 0.20 (alarm a 30% → a ≥60% route drop, frac<0.40). The 0.10 was set to the R29
  probe's rolling-Qn ~5% separation but fired ~21% on the anchored full seed (chronic
  route-withdrawal states, not withdrawal events); 0.20 fires 8.7% (145/1673 down-trembles) and keeps
  both cited events (Syria 2022-05-30 z −4.84, Sudan 2023-04-24 z −3.43; 0.20 is the chosen value —
  any materiality ≤ ~0.229 still fires Sudan, 0.23 or tighter drops it). This resolved the base-rate
  over-fire ONLY — chronic states SY/IQ/SD still drive ~72% of fires (up from ~48%), so chronic-state
  handling REMAINS the pre-promotion calibration item (base-rate item resolved, not closed). The
  committed seed was re-scored at 0.20 — raw_value (the measurement) unchanged, only z_score/trembling
  recompute; a forward-only method annotation records the z-score change (pre-correction 0.10 scoring
  preserved at commit 7f0eb08). tier-2 uncounted, whole seed before STABLE_SINCE — replay --check
  0-divergence, no STABLE_SINCE bump, no summary change. radar-metrics regenerated.
- **Round 32** — 2026-09-10 · the factor-model round: two probes run, both FAIL, and the DOCTRINE is
  sedimented (radar.md gains "tremor is a factor library for world disorder"). The factor-lens review
  read the roster as a quant model (credit_spread + fed_srf = live alpha; flights = a stale aggregate
  to reconceive per-region ~Nov; cnh_cny = a reference-regime axis; net_bgp = a chronic beta;
  ENERGY the un-spanned dimension — with the correction that sovereign-solvency co-moves with credit
  +0.80, so energy, not sovereign, was the real gap). **Probe A (net_bgp chronic neutralization) FAILS:**
  SY/IQ/SD dominate the fires because they ARE the most chronically-disordered route tables — the
  factor's distribution, not a subtractable beta; no level residual reaches both fresh AND
  event-preserving (a short window loses Sudan, a long one leaves lag-1 0.49–0.52, above the tier-1
  0.27–0.41 band). A breadth/per-episode cap is unlikely (Sudan is single-country) and would be a
  different line; comms slot stays open, and net_bgp is held tier-2 (past 60 scored but chronic-dominated,
  not promotion-ready). **Probe B (build entsog_gas_flow, the energy axis) FAILS on the IC gate →
  REJECTED:** the worst-of/own-baseline construct is degenerate for lumpy gas flow (single-day fires
  97.5%, median worst-frac 0.00). fable R32 corrected the salvage narrative: its watch-lists were floored
  ex-post on a recent window, survivorship-excluding the very corridors whose cut it claimed to test — a
  lookahead; it tested LNG/VIP lumpiness, not cut-distinguishability. The build was fully reverted (no
  registration); energy is left un-spanned per **Coverage is not a goal**, but UNTESTED not un-spannable —
  the cheapest untried construct (a single summed EU-total or Russia-corridor series, no adjacency needed)
  was NOT run this round per the step-back. The doctrine's sharpening — coverage is not a goal, spanning
  is a direction not a quota, tier-1's 4 is a display budget, an axis is un-spanned only after the cheapest
  single-series construct is base-rate tested — is R32's durable output. Docs-only; no line moved, no
  scoring changed (replay --check 0-divergence, no STABLE_SINCE bump). Process note + new RULE: an
...
- **Round 34** — 2026-09-13 · the flights airspace-reconception, REJECTED on coverage (no line built).
  The factor-lens flagged `flights` as a stale weather-suppressing aggregate; the round tried to
  reconceive it into a probe for the invisible HUMAN hand (airspace closures) and, through a full
  measure-first arc, found the axis un-spannable on the tested conflict zones (Ukraine, Israel) via keyless community ADS-B. Per-region worst-of →
  rejected (the "Typhoon Dolphin" recall was a sample-hour phase seam, precision 0/15). A
  conflict-corridor closure construct → probed, coverage-validated (5 boxes), planned, fable-reviewed
  DON'T-BUILD (250-nm box/neighbor circles overlap 31–64%, so box÷neighbor is self-over-self), then
  redesigned to a polygon/ring and BACKTESTED on keyless historical snapshots — decisively rejected:
  ADS-B coverage is structurally absent over conflict zones (central Ukraine ~1 airborne pre-war AND
  during-war = a coverage floor; the live "Ukraine=1" was that floor, not a closure). `flights` stays
  as-is (user-confirmed). Recorded in Rejected + the two flights record-integrity fixes (the Nov
  per-region tripwire KILLED — its −7.84/−7.33z were phase-seam artifacts; the R25.1 note scoped to the
  total, not per-region shares). Two confident positives (typhoon recall, Ukraine signal) were retracted
  on measurement — the discipline caught both before any build. Method banked: adsbexchange readsb-hist
  keyless historical snapshots for future backtests. Docs-only; no line moved, no scoring changed
  (replay --check 0-divergence, no STABLE_SINCE bump). radar-metrics regenerated.
